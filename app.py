...
if st.button('Calculate Optimal Strategy'):
    ...
    if result:
        ...
        with col1:
            ...
            st.metric('Capital Spot', f"{result['Capital_Spot']:.2f} USDT")
            st.metric('Capital Futures', f"{result['Capital_Futures']:.2f} USDT")
            st.metric('Borrow Amount', f"{result['Borrow_Amount']:.2f} USDT")
        
        with col2:
            st.subheader('Position Details')
            st.metric('ETH Initial', f"{result['ETH_Initial']:.4f} ETH")
            st.metric('ETH Borrowed', f"{result['ETH_Borrowed']:.4f} ETH")
            st.metric('Total ETH', f"{result['Total_ETH']:.4f} ETH")
        
        with col3:
            st.subheader('Performance Metrics')
            st.metric('ROI', f"{result['ROI']*100:.2f}%")
            st.metric('Net Income', f"{result['Net_Income']:.2f} USDT")
            st.metric('LTV', f"{result['LTV']*100:.2f}%")
        
        # Create two columns for leverage and liquidation details
        col4, col5 = st.columns(2)
        
        with col4:
            st.subheader('Leverage')
            st.metric('Spot Leverage', f"{result['Leverage_Spot']:.2f}x")
            st.metric('Futures Leverage', f"{result['Leverage_Futures']:.2f}x")
        
        with col5:
            st.subheader('Liquidation Prices')
            st.metric('Spot Liquidation', f"${result['Liq_Price_Spot']:.2f}")
            st.metric('Futures Liquidation', f"${result['Liq_Price_Futures']:.2f}")
            st.metric('Spot Distance', f"{result['Liq_Distance_Spot_%']:.2f}%")
            st.metric('Futures Distance', f"{result['Liq_Distance_Futures_%']:.2f}%")
    else:
